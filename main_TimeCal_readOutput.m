...
var_names = {'${D}_{\mathrm{c}}$','$\dot{\overline{\gamma}}_t$'};
%% Read input file
myVars = {'INPUT','exp_design'};
load('timeCal2_00input_100sims_2Jun22.mat',myVars{:})
Ns = size(exp_design,1);
M = size(exp_design,2);

%% Output
folderPath = 'TimeCal2_validation';
...
destinationSave = 'M:\IFM\User\melito\PhD\Projects\TimeCalibration_storageNoGitHub_saveFiles\Plot_AliModel_Calibration2_valid100';
try
    cd(destinationSave)
catch
...
save('RawOutput_06June22_100sim.mat','-v7.3')
cd(root_destination)
