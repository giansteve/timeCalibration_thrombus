function [outputToPCE] = timeCal_getOutputReadable(OUTPUT,thresholdThr)
...
for nsim = 1:length(OUTPUT)
    ...
    %% Time
    outputToPCE.time(:,nsim) = OUTPUT(nsim).time(1000:50:end);
    ...
    outputToPCE.H_S(:,nsim) = OUTPUT(nsim).H_S.output(1000:50:end);
    ...
    outputToPCE.L_S(:,nsim) = OUTPUT(nsim).L_S.output(1000:50:end);
